{% macro customer_order_vs_trg(trg_table,column_base,column_foreign,column_to_checked,column_condition)%}
    ...
    trg_table: shipped_status, column_base: ID, column_foreign: user_id, column_to_checked: status,column_condition: shipped
    */
     --Suite start time
    {%set suite_time%}
    ...
    -- Test Case : joined table to check shipping status as shipped it has some extra arguments which are because of two tables being joined
